...
function lilyVoice(part,voice){let out=[];for(let m=0;m<state.measures;m++){let cur=0,t=[],events=state.notes.filter(n=>n.part===part&&n.voice===voice&&n.measure===m).sort((a,b)=>a.beat-b.beat||a.step-b.step),groups=[];for(const n of events){let g=groups.find(g=>g[0].beat===n.beat);g?g.push(n):groups.push([n])}for(const g of groups){const first=g[0];if(first.beat>cur)t.push(...gaps(first.beat-cur));const sounding=g.filter(n=>!n.rest),token=sounding.length>1?'<'+sounding.map(n=>lilyPitch(n.pitch)).join(' ')+'>':sounding.length?lilyPitch(sounding[0].pitch):'r';t.push(token+lilyDur(first));cur=Math.max(cur,first.beat+Math.max(...g.map(n=>n.duration*(n.dotted?1.5:1))))}if(cur<beats())t.push(...gaps(beats()-cur));out.push(t.join(' ')+' |')}return out.join('\n    ')}
function lilySource(){const km={'C major':'c \\major','G major':'g \\major','D major':'d \\major','F major':'f \\major','E♭ major':'ees \\major'},heads=state.shapedNotes?'\\aikenHeads ':'';let defs='',staves='';parts().forEach((p,i)=>{const id='part'+(i+1),lyrics=state.notes.filter(n=>n.part===i&&n.voice===1&&!n.rest&&n.lyric).sort((a,b)=>a.measure-b.measure||a.beat-b.beat).map(n=>n.lyric.replace(/-/g,' -- ')).join(' ');defs+=`${id}One = { ${heads}\\key ${km[state.key]} \\time ${state.time} \\tempo 4 = ${state.tempo}\n    ${lilyVoice(i,1)}\n}\n${id}Two = { ${heads}${lilyVoice(i,2)} }\n${id}Lyrics = \\lyricmode { ${lyrics} }\n\n`;staves+=`    \\new Staff \\with { instrumentName = "${p[1]}" } <<\n      \\clef ${p[2]} \\new Voice = "${id}" { \\voiceOne \\${id}One }\n      \\new Voice { \\voiceTwo \\${id}Two }\n      \\new Lyrics \\lyricsto "${id}" { \\${id}Lyrics }\n    >>\n`});return `\\version "2.24.0"\n\\header { title = "${state.title.replace(/"/g,'\\\"')}" tagline = ##f }\n\n${defs}\\score {\n  \\new ChoirStaff <<\n${staves}  >>\n  \\layout { }\n  \\midi { }\n}\n`}
